...
Доступ к API осуществляется с помощью JWT-токена. Токен можно получить внутри панели управления GoSMS в разделе [API интеграция](https://cms.gosms.ru/api). Токен необходимо передавать в заголовке каждого запроса в формате: 
